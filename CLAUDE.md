...
Warm-parchment editorial system, light only (tokens in `site/src/styles/global.css`). Use role tokens (`bg-canvas`, `bg-card`, `bg-feature`, `bg-panel`, `text-ink`, `border-line`), never raw hex. Self-hosted Source Serif 4 (display headings at weight 400 with tight tracking, and body copy) and Inter (UI chrome, card titles, labels), plus JetBrains Mono for code, all via Fontsource. Never set headings in bold sans. No shadows, gradients or glows: surfaces get their depth from tone and 1px borders only. The clay `bg-action` button appears at most once per page, with dark text (light text on clay fails WCAG AA). Filled secondary buttons round only their bottom corners (`rounded-b-lg`). Inline links always show their underline.
